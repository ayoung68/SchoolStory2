...
hist.kills <- (hist(LOL.train$Kills, xlab = 'Kills', ylab = 'Frequency'))
hist.assists <- (hist(LOL.train$Assists, xlab = 'Assists', ylab = 'Frequency'))
hist.deaths <- (hist(LOL.train$Deaths, xlab = 'Deaths', ylab = 'Frequency'))
hist(LOL.train$Deaths)

# EDA
#hist(Kills, Win, data = LOL.train) 
#hist(Kills$LOL.train)
...
x.star<-data.frame(Kills=seq(0,10,length=100), Deaths=2,Assists=6)
...
x.star.Deaths<-data.frame(Deaths=seq(2,10,length=100), Kills=2,Assists=6)
...
x.star.Assists<-data.frame(Assists=seq(3,13,length=100), Deaths=2,Kills=2)
