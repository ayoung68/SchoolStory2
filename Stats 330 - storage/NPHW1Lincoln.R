...
#(c)
#Create a dataframe called lincoln.
#Lincoln Memorial NP, (annual visitors since 1936)
#Follow this data link: https://irma.nps.gov/Stats/SSRSReports/Park%20Specific%20Reports/Annual%20Park%20Recreation%20Visitation%20(1904%20-%20Last%20Calendar%20Year)
#retrieved on Thu Jan 18 2018

lincoln <- read.csv(header = TRUE, text = '
                Year,RecreationVisitors,TotalRecreationVisitors
...
#remove the commas from the numbers
lincoln$RecreationVisitors<-as.numeric(gsub(',','',lincoln$RecreationVisitors))
mean(lincoln$RecreationVisitors)

#change to "Visitors (in millions)"
lincoln$RecreationVisitors<-lincoln$RecreationVisitors/10^6

#EDA

# plot time series
plot(RecreationVisitors~Year, data = lincoln, type='b',
     ylab = "Lincoln Memorial NP Annual Visitors (in millions)")
tail(lincoln)

#Due to a considerable drop in annual visitors to Lincoln Memorial NP during the 1990s,
#it appears that the assumption of additivity is violated with this data. The data shows 
#evidence of a multiplicative effect, meaning a transformation may be neccessary.

#(e)
#A log transformation seems necessary since the data varies so much in the 1990s.
lincoln$lnVisitors <- log(lincoln$RecreationVisitors)

#Confirm the relationship appears more additive.
plot(lincoln$lnVisitors~Year, data = lincoln, type = "b")
lincolnnew <- lincoln[20:81,]

#(f) Modeling
# estimate ARIMA(1,1,1)
library(astsa)
lincolnnew.out <-sarima(lincolnnew$lnVisitors,1,1,1)
lincolnnew.out$ttable

# compute predictions for next 5 years
lincolnnew.future<-sarima.for(lincolnnew$lnVisitors,n.ahead=5,1,1,1)
lincolnnew.future

#compute 95% prediction intervals
lincolnnew.future.L<-lincolnnew.future$pred - qnorm(0.975) * lincolnnew.future$se
lincolnnew.future.U<-lincolnnew.future$pred + qnorm(0.975) * lincolnnew.future$se

# un-transform
lincolnnew.yhat<-exp(lincolnnew.future$pred)
lincolnnew.yhat.L<-exp(lincolnnew.future.L)
lincolnnew.yhat.U<-exp(lincolnnew.future.U)

cbind(lincolnnew.yhat,lincolnnew.yhat.L,lincolnnew.yhat.U)
...
plot(RecreationVisitors~Year, data = lincolnnew, type = "b",
     ylab = "Lincoln Memorial NP Annual Visitors (in millions)",
     xlim = c(1955, 2021), ylim = c(1,8.9))
lines(2017:2021, exp(lincolnnew.future$pred), col="darkorange2", type = "b", pch = 19)
lines(2017:2021, exp(lincolnnew.future.L),col="darkorange2",lty =2) 
lines(2017:2021, exp(lincolnnew.future.U),col="darkorange2",lty =2) 
