...
#hills$Time, hills$Race
# Yes. Based on the correlation coefficient (0.7875876) and scatterplot, the data is normally distributed, and
# there appears to be a mostly strong association between temperature and carbon dioxide concentration.
...
# model temp: = beta0 + beta1 co2 + beta2 methane + beta3FFFFFF + beta5 sf6 + epsilon + epsilon~N(0,sigma2)
...
subset(out.climate, leverage > 2*(5+1)/189)



# Based no the data, it appears that n2o and sf6 affect the collinearity the most.Due to collinearity involving
# these two variables, it appears that it would be reasonable to ignore the interaction with each of these variables
# in order to avoid seeing the conflicting plots for explanatory variables.

leverage.climate<-lm.influence(out.hills)$hat
subset(leverage.hills,hills$race=="BenNevis")

# We decided to throw out the n2o and sf6 data in order to see the data without influential observations or outliers.
# This allows us to fit a model with data that fit the model.
  #VIF
  library(car)
  vif(out.climate2)
