...
plot(temp~n2o, data = climate,
xlab = "n2o",
ylab = "Temperature")
  
  cor(climate$temp, climate$hcfc)
  
  plot(temp~hcfc, data = climate,
       xlab = "hcfc",
       ylab = "Temperature")
  
  cor(climate$temp, climate$sf6)
  
  plot(temp~sf6, data = climate,
       xlab = "sf6",
       ylab = "Temperature")
  ...
  # Yes, based on the coefficient (0.7590174) and scatterplot, there appears to be a mostly strong association
  # between temperature and carbon dioxide concentration, and the data is normally distributed. 
  
  # The response variable is the global average temperature. The explanatory variables are the methane and carbon dioxide levels.  
  
  # Yes, based on the correlation coefficient of temp and each of the explanatory variables, there is a moderately
  # strong relationship between temperature and n20, mcfc, and sf6 concentration. The data is normally distributed.
  
  # Model temp = beta0 = beta1 co2 = beta2 methane = epsilon, epsilon~N9
  # Assumptions for inference: Linear, strong correlation, no multiplicative effect, few outliers.
  out.climate <- lm(temp~co2+methane, data = climate)
  
  summary(out.climate)
  
  # The value B^1 represents the partial slope or mean, which is that for a one unit change in co2 (ppm), we estimate
  # an increase in global temperature of 0.741 (.01 degrees celsius), holding co2 constant.
  
  # The value B^2 represents the partial slope or mean, which is that for a one unit change in methane (ppb), we estimate
  # an increase in global temperature of 0.094 (0.01 degrees C), holding methane constant.
  
  
  # Climate Change Regression 3: Analysis
  
  # The response variable is temperature. The explanatory variables are the n2o, hydrobchloroflurocarbon, and sulfur
  # hexaflouride concentrations.
  
  # Fit the model: Model temp = beta0 = beta1 co2 = beta2 methane = epsilon, epsilon~N9
  # model temp: = beta0 + beta1 co2 + beta2 methane + beta3n2o + beta4hcfc beta5sf6 + epsilon + epsilon~N(0,sigma2)
  out.climate2 <- lm(temp~co2+methane+n2o+hcfc+sf6, data = climate)
  summary(out.climate2)
    
  #Because of the non-sensical regression coefficients, let's explore collinearity
...
leverage.climate<-lm.influence(out.climate2)$hat
subset(out.climate2, leverage.climate > 2*(5+1)/223)

par(mfrow=c(1,2))
plot(x=climate$n2o, y=climate$temp)
points(climate$n2o[76], climate$temp[76], col= "blue2", pch=19)

plot(x=climate$sf6, y=climate$temp)
points(climate$sf6[32], climate$temp[32], col= "red", pch=19)
par(mfrow=c(1,1))

  
# Based on the data, it appears that n2o and sf6 affect the collinearity the most. Due to collinearity involving
# these two variables, it appears that it would be reasonable to ignore the interaction with each of these variables
# in order to avoid seeing the conflicting plots for explanatory variables and influential observations.
  
subset(leverage.climate, out.climate2$n2o=="1:189")
subset(leverage.climate, out.climate2$sf6=="1:189")
  
#VIF
  library(car)
  vif(out.climate2)
  
  #The data show leverage.
    
  # partial effect plots
  library(car)
  avPlots(out.climate2)
  
  # After subsetting the data to address the collinearity and data issues, we threw out the sf6 and n2o
  # variables to remove the collinearity effect. This was evident in the final VIF plots, which revealed a value 
  # of 375.0616 for n2o and a value of 373.859809 for sf6. Given that a VIF value greater than 10 is typical to 
  # be thrown out, it seems correct to throw out each of these explanatory variables. This removes the 
  # confounding effect which comes from potential lurking variables.
  
  # Research Task and Data Features that Match Analysis Strengths:
  # The VIF plots and subsetting of the data helped mitigate the collinearity effect. These helped
  # to view the modified data, showing the true effect of each of the explanatory variables on the
  # response variable. The correlation coefficients also showed how well the data was distributed, 
  # which initially showed a strong positive correlation.
  
  # Analysis Weaknesses:
  # The main weakness within the analysis was the collinearity that showed up within the n2o and sf6 variables.
  # This confounding is a weakness of having multiple explanatory variables in an experiment.
  

  #CP PART 1
  # Test Ho: no greenhouse gas effect'
  # fit model assuming Ho is true
  nogreenhousegas.climate<-lm(temp~+1, data = climate)
  anova(nogreenhousegas.climate, out.climate)
  
  # ANOVA test
  # The null hypothesis (Ho) is that there is no relationship between greenhouse gases and average global temperature.
  # The alternative hypothesis (Ha) is that there exists a relationship between greenhouse gases and average global temperature.
  # F-test statistic value = 338.47, P-value = 2.2e-16. 
  # With an F-value of 338.47 and a p-value of p<.001, we have evidence to claim that there is a statistically significant
  # greenhouse gas effect. In other words, there is a strong effect of greenhouse gases such as carbon dioxide and methane
  # on global average temperatures.
  
  # since we reject Ho and conclude there is a  significant greenhouse gas effect, look at the individual regression coefficients
  # t-tests from summary
  # 95% CI
  confint(out.climate)
  
  # CI
  # Co2
  # Based on the results, we are confident that in repeated sampling, the true mean temperature will fall in the interval 
  # (0.54625793, 0.9358747) 95% of the time. In other words, we are 95% confident that if multiple observations were taken,
  # the mean temperature value will lie in the interval (0.54625793, 0.9358747).
  
  # Methane
  # According to the results, in repeated sampling, we are confident that the true mean temperature will fall in the interval (0.02998332, 0.1588980) 
  # 95% of the time. In other words, we are 95% confident, if many observations were taken, that the mean temperature value will lie in the 
  # interval (0.02998332, 0.1588980).
  
  # According to these intervals, it is CO2 which contributes most to the greenhouse gas effect on temperature.
  ...
  predict(out.train, newdata = test.climate)
  
  # compare actual to prediction
  cbind(test.climate$temp,predict(out.train,newdata=test.climate))
  
  # graphic
  plot(predict(out.train,newdata=test.climate),test.climate$temp)
  abline(0,1,col="gray")
  
  # results above shown in degrees celsius.
  
  # Research Task and Data Features that Match Analysis Strengths:
  # One feature of the data which match the analysis strength was a linear relationship, based on the correlation
  # coefficient. The prediction showed that the majority of the greenhouse gas effect came from carbon dioxide. This 
  # a strength of the analysis.
  
  # Weaknesses:
  # One weakness of the analysis was that based on the absolute prediction error, the prediction overestimated
  # the actual global temperature of the greenhouse gases. 
